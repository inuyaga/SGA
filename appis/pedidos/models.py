...
class Producto(models.Model):
    ...
    producto_imagen=models.ImageField(blank=True, null=True, upload_to="cover/%Y/%m/%D", verbose_name='Imagen')
    ...
    producto_precio = models.FloatField(null=True, blank=True, verbose_name='Precio')
...
#Obtiene nombre de la sucursal
    def nombre_sucursal(self):
        query=Departamento.objects.get(departamento_id_depo=self.pedido_id_depo.departamento_id_depo)
...
#Obtiene nombre de la empresa
    def empresa(self):
        query=Sucursal.objects.get(id_sucursal=self.nombre_sucursal().id_sucursal)
        return query.sucursal_empresa_id
    def total(self):
        total_importe=Detalle_pedido.objects.filter(detallepedido_pedido_id=self.pedido_id_pedido).aggregate(total=Sum(F('detallepedido_cantidad')*F('detallepedido_precio')))
...
class Detalle_pedido(models.Model):
    ...
    detallepedido_creado_por=models.ForeignKey(User, null=True, blank=True, on_delete=models.PROTECT)
