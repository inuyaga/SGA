...
class ProductoUpdateBulk(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for item in excel_data:
            Producto.objects.filter(producto_codigo=item[0]).update(
...
class ProductoCompraList(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['conteo'] = Detalle_pedido.objects.filter(
            detallepedido_creado_por=self.request.user, detallepedido_status=False).count()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if Decimal(cantidad) <= 0:
            ...
            try:
                ...
                suma_pedido = Detalle_pedido.objects.filter(detallepedido_creado_por=request.user, detallepedido_status=False, detallepedido_tipo_pedido=tipo_pedido).aggregate(
                    suma_total=Sum(F('detallepedido_precio') * F('detallepedido_cantidad')))
                # SUMAMOS LA SUMA DEL PEDIDO CON EL PRODUCTO ACTUAL ÁRA AGREGAR
                subtotal = (Decimal(producto.producto_precio) * Decimal(cantidad)) + Decimal(
                    0 if suma_pedido['suma_total'] == None else suma_pedido['suma_total'])
                ...
                if subtotal <= limite_gasto:
                    ...
                    tipo_mensaje = False
                    mensaje = 'Supera el limite admitido para ' + \
                        str(get_tipo_pedido.tp_nombre)
        ...
        pedido_conteo = Detalle_pedido.objects.filter(
            detallepedido_creado_por=request.user, detallepedido_status=False).count()
        json = JsonResponse(
            {
                'content': {
                    'mensaje': mensaje,
...
class Crear_pedido_tiendaView(View):
    def get(self, request, *args, **kwargs):
        ...
        conteo_tip_ped = Detalle_pedido.objects.filter(
            detallepedido_creado_por=request.user, detallepedido_status=False, detallepedido_tipo_pedido=tipo_pedido).count()
        if conteo_tip_ped > 0:
            pedido = Pedido(pedido_id_depo_id=departamento,
                            pedido_tipoPedido_id=tipo_pedido)
            pedido.save()
            Detalle_pedido.objects.filter(detallepedido_creado_por=request.user, detallepedido_status=False, detallepedido_tipo_pedido=tipo_pedido).update(
                detallepedido_pedido_id=pedido.pk, detallepedido_status=True)
...
class PedidoList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'autoriza' in urls_formateada:
            Pedido.objects.filter(pedido_id_pedido=self.request.GET.get('autoriza')).update(
                pedido_status=2, pedido_autorizo=self.request.user)
            del urls_formateada['autoriza']

        if 'cancela' in urls_formateada:
            Pedido.objects.filter(pedido_id_pedido=self.request.GET.get('cancela')).update(
                pedido_status=3, pedido_rechazado=self.request.user)
        ...
        if inicio != None and fin != None:
            if inicio != '' and fin != '':
                queryset = Pedido.objects.filter(
                    pedido_fecha_pedido__range=(inicio, fin))

        if sucursal_selec != None:
            if sucursal_selec != '0':
                queryset = queryset.filter(
                    pedido_id_depo__departamento_id_sucursal=sucursal_selec)
...
class dowload_pedido_detalles(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        query = Departamento.objects.get(
            departamento_id_depo=ped.pedido_id_depo.departamento_id_depo)
        nombre_sucursal = query.departamento_id_sucursal

        ws['A1'] = 'Detalle Pedido ' + \
            str(nombre_sucursal)+' N°' + str(id_pedido)
        st = ws['A1']
        ...
        ws['F2'] = 'Total'
        cont = 3
        detalle = Detalle_pedido.objects.filter(
            detallepedido_pedido_id=id_pedido)

        for cto in detalle:
            ws.cell(row=cont, column=1).value = str(
                cto.detallepedido_producto_id)
            ws.cell(row=cont, column=2).value = str(
                cto.detallepedido_producto_id.producto_descripcion)
            ws.cell(row=cont, column=3).value = str(
                cto.detallepedido_pedido_id.pedido_id_depo)
            ...
            ws.cell(row=cont, column=6).value = (
                Decimal(cto.detallepedido_cantidad) * Decimal(cto.detallepedido_precio))
        ...
        Pedido.objects.filter(pedido_id_pedido=id_pedido).update(
            pedido_status=7, pedido_Descargado=request.user)
...
class PedidoUpdate(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if form.instance.pedido_status == 2:
            ...
            Detalle_pedido.objects.filter(detallepedido_pedido_id=self.kwargs.get(
                'pk')).update(detallepedido_status=False)
        return super().form_valid(form)
...
class PedidoListSucursal(ListView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            messages.warning(
                self.request, 'Actual mente usted no pertenece a un departamento, por lo tanto no puede visualizar pedidos realizados.')
    ...
    def get(self, request, *args, **kwargs):
        ...
        if inicio != None and fin != None:
            if status == '0':
                ...
                query_list = query_list.filter(
                    pedido_fecha_pedido__range=(inicio, fin), pedido_status=status)
        # else:
        #     if status == '0':
        #         query_list=Pedido.objects.all()
        #     else:
        ...
        for cto in query_list:
            ...
            ws.cell(
                row=cont, column=6).value = 'NO' if cto.pedido_autorizo == None else cto.pedido_autorizo.username
            ws.cell(
                row=cont, column=7).value = 'NO' if cto.pedido_rechazado == None else cto.pedido_rechazado.username
            ws.cell(
                row=cont, column=8).value = '' if cto.pedido_n_factura == None else cto.pedido_n_factura
            ws.cell(
                row=cont, column=9).value = '' if cto.pedido_n_cresscedo == None else cto.pedido_n_cresscedo
...
class DowloadDetallesporPedido(View):
    def get(self, request, *args, **kwargs):
        ...
        query = Detalle_pedido.objects.filter(
            detallepedido_pedido_id__pedido_fecha_pedido__range=(inicio, fin))

        if sucursal_selec != '0':
            query = query.filter(
                detallepedido_pedido_id__pedido_id_depo__departamento_id_sucursal=sucursal_selec)
        if status != '0':
        ...
        ws['I2'] = 'Subtotal'
        cont = 3
        colors_rbga = ['EFA8D3', 'BEABF6', 'F3B8F8', '6FE3F5', 'F4B2B2', 'FFB7D5', 'A7ABFA', '94F4B5',
                       'F7D896', 'D4D3D3', 'FFBCBC', 'F8CEA4', 'CEF683', 'C8FBC0', '87C8E2', 'A7C4FA', 'F49495', 'D4B1EB']
        tem_cont_ped = 0
        ...
        for cto in query:
            ...
            # COLOREAR CELDAS
            ws.cell(row=cont, column=1).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=2).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=3).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=4).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=5).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=6).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=7).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=8).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            ws.cell(row=cont, column=9).fill = PatternFill(
                start_color=color_temporal_gen, end_color=color_temporal_gen, fill_type='solid')
            # INCRUSTAR CONTENIDO DE CELDA
            ws.cell(
                row=cont, column=1).value = cto.detallepedido_pedido_id.pedido_id_pedido
            ws.cell(row=cont, column=2).value = str(
                cto.detallepedido_pedido_id.pedido_id_depo.departamento_nombre)
            ws.cell(row=cont, column=3).value = str(
                cto.detallepedido_pedido_id.pedido_id_depo.departamento_id_sucursal)
            ws.cell(row=cont, column=4).value = str(
                cto.detallepedido_pedido_id.pedido_tipoPedido)
            ws.cell(row=cont, column=5).value = str(
                cto.detallepedido_producto_id)
            ws.cell(row=cont, column=6).value = str(
                cto.detallepedido_producto_id.producto_nombre)
            ws.cell(row=cont, column=7).value = cto.detallepedido_cantidad
            ws.cell(row=cont, column=8).value = cto.detallepedido_precio
            ws.cell(row=cont, column=9).value = '=PRODUCT(G' + \
                str(cont)+':H'+str(cont)+')'
...
class AsigGastoList(ListView):
    ...
    def get_queryset(self):
        ...
        if sucursal != None:
            queryset = queryset.filter(
                ags_sucursal__departamento_id_sucursal=sucursal)
        return queryset
...
class PDFCatalogoProd(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        doc = SimpleDocTemplate(buff, pagesize=letter, rightMargin=40, leftMargin=40,
                                topMargin=60, bottomMargin=18, title='CATALOGO PRODUCTOS')
        items = []
        data_tabla = []
        ...
        stylo_p = ParagraphStyle(
            'parrafo', alignment=TA_LEFT, fontSize=11, fontName="Times-Roman")
        stylo_titulo = ParagraphStyle(
            'titulo', alignment=TA_CENTER, fontSize=11, fontName="Times-Bold")
...
class InventarioBusqueda(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if buscar != '':
            if buscar != None:
                context['produsctos'] = Producto.objects.filter(Q(producto_descripcion__icontains=buscar) | Q(
                    producto_codigo__icontains=buscar) | Q(prducto_codigo_barras__icontains=buscar), producto_visible=False)
        return context
...
class InvCapturaResguardo(CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            form.instance.inv_producto = Producto.objects.get(
                producto_codigo=form.cleaned_data['inv_producto'])
            self.object = form.save()
            messages.success(self.request, 'Agregado')
            return super().form_valid(form)
        except IntegrityError as error:
            messages.warning(
                self.request, 'Producto y ubicacion duplicado intente con otra ubicacion'+str(error))
...
class InventarioAvanceConteo(ListView):
    ...
    def get_queryset(self):
        ...
        if buscar != '':
            if buscar != None:
                queryset = queryset.filter(Q(inv_producto__producto_descripcion__icontains=buscar) | Q(
                    inv_producto__producto_codigo__icontains=buscar), inv_producto__producto_visible=False)[:100]
        return queryset
...
class InventarioAvanceConteoGlobal(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Inventario.objects.values('inv_producto', 'inv_producto__producto_descripcion', 'inv_producto__prducto_existencia').filter(inv_validacion=True, inv_conteo=tipo_conteo).annotate(
            ...
            suma_total=Sum(F('inv_cant_resguardo')) +
            Sum(F('inv_cant_piking'))+Sum(F('inv_cant_otros')),
        ).order_by('inv_producto')
        return queryset

    def get_context_data(self, **kwargs):
        context = super(InventarioAvanceConteoGlobal,
        ...
        context['total_productos'] = Producto.objects.filter(
            producto_visible=False).count()
        avance_porcentaje = (
            context['total_conteo_prod']*100)/context['total_productos']
...
class ValidarConteoMerma(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['inventario'] = Inventario.objects.filter(
            inv_sup_autorizo_merma=self.request.user, inv_validacion=False)
...
class DowloadInventarioGlobal(View):
    def get(self, request, *args, **kwargs):
        ...
        for cto in query:
            ...
            ws.cell(
                row=cont, column=2).value = cto['inv_producto__producto_descripcion']
            ...
            ws.cell(
                row=cont, column=8).value = cto['inv_producto__prducto_existencia']
...
class DowloadReporteCapturaXhora(View):
    def get(self, request, *args, **kwargs):
        ...
        report_hora_captura = cont_inv.values('hora', 'dia').annotate(
            conteo=Count('*')).order_by('dia', 'hora')
        cont = 3
        for cto in report_hora_captura:
            # INCRUSTAR CONTENIDO DE CELDA
            ws.cell(row=cont, column=1).value = localize(cto['dia'])
            ws.cell(
                row=cont, column=2).value = cto['hora'] if cto['hora'] > 0 else cto['hora'] * -1
